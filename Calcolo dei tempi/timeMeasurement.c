...
int calcolaRip( double (**P)(double*, int), int functionNumber, double* array, int size, long long tMin );

long long tempoMedioNetto( double (*prepara)(double*, int), double (*P)(double*, int), double* array, int size, long long tMin);

void misurazione(double (*prepara)(double*, int), double (*P)(double*, int), double* array, int size, int c, long long tMin, double za, struct measureSpec *retval);

int main(int argc, char** argv){

    int endSize = ARRAY_START_SIZE + ARRAY_NEXT_SIZE_INCREMENT * (NUMBER_OF_ITERATIONS - 1);
    ...
    for( int i = 0; i < NUMBER_OF_ITERATIONS; i++ )
    {
        ...
        misurazione(setupArray, weightedMedianDeterministic, array, current, C, tMin, TIME_ZA, &retDETERMINISTICO);

#ifdef DO_ALL
        misurazione(setupArray, weightedMedianRandom, array, current, C, tMin, TIME_ZA, &retRANDOM);
        misurazione(setupArray, weightedMedianQuickSort, array, current, C, tMin, TIME_ZA, &retQUICKSORT);
        misurazione(setupArray, weightedMedianInsertionSort, array, current, C, tMin, TIME_ZA, &retINSERTIONSORT);
 ...
 * In ambiente windows si possono utilizzare le API per la crittografia (CryptGenRandom).
 */
void setSeed()
{
    srand(time(NULL));
...
long long getGranularity()
{
    ...
    }while( (timeStart.tv_nsec == timeEnd.tv_nsec) && (timeStart.tv_sec == timeEnd.tv_sec));
    return (timeEnd.tv_sec - timeStart.tv_sec) * (TIME_SECTONSEC_MULTIPLIER) + (timeEnd.tv_nsec - timeStart.tv_nsec);
}
...
void misurazione(double (*prepara)(double*, int), double (*P)(double*, int), double* array, int size, int c, long long tMin, double za, struct measureSpec *retval)
{
    ...
    do
    {
        ...
        cn = cn + c;
        retval->value = t/cn;
        s = sqrt( (double)((unsigned long long)(sum2/cn) - (unsigned long long)(retval->value * retval-> value) ) );
